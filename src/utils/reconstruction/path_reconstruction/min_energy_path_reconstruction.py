...
class SquaredMinEnergyPathReconstructionMethod(MinEnergyPathReconstructionMethod):
    def __init__(self, up_height_factor = 1.0, 
                 down_height_factor = 1.0, 
                 connexity = Connexity.CONNEX_8
    ) -> None:
        super().__init__(connexity=connexity)
        self.up_height_factor = up_height_factor
        self.down_height_factor = down_height_factor
...
class MultiChannelSquaredMinEnergyPathReconstructionMethod(PathReconstructionMethod):
    def __init__(self,
                 up_height_factor = 1.0, 
                 down_height_factor = 1.0, 
                 connexity = Connexity.CONNEX_8
    ) -> None:
        super().__init__(height_related=True)
    ...
    def dijkstra_heightmap(self,
        ...
        while pq:
            ...
            for dy, dx in neighbors:
                ...
                cost = self.energy_function(heightmaps, x, y, nx, ny)
    ...
    def reconstruct_one(self,
        ...
        dist, prev = self.dijkstra_heightmap(heightmaps.numpy(), pos_start, pos_goal)
        path = self.reconstruct_path(prev, pos_start, pos_goal)
        return path
