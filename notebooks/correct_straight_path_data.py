...
new_centerlines_folder = os.path.join(data_folder, "straight_centerlines")
...
def process_case(i):
    ...
    print(f"Number of positive samples: {len(true_edges_classes)}")

    display_graph_overlay(np.zeros_like(gt_np), in_pred_graph, figsize=(40,40))
    ...
    print(f"Number of negative samples: {len(train_negative_classes)}")
    ...
    img_background[segmentation_mask_np] = [255,125,0]
    img_background[gt_np] += np.array([0, 125, 255], dtype=np.uint8)
