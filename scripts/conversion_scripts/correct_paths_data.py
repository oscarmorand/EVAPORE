...
def clean_paths_on_surface_of_mask(centerlines, mask, threshold=0.5):
    new_centerlines = []
    for centerline in centerlines:
        centerline_mask = np.zeros_like(mask, dtype=bool)
        for (x, y) in centerline:
...
def process_case(i):
    ...
    centerlines_train_data = centerlines_data['train']
    centerlines_train_centerlines = centerlines_train_data['path_centerlines']
    centerlines_train_edges_classes = centerlines_train_data['edges_classes']
    centerlines_train_negative_centerlines = [cl for cl, c in zip(centerlines_train_centerlines, centerlines_train_edges_classes) if c == 0]
    ...
    true_path_reconstructed_centerlines = cut_mask_from_negative_edges_for_all(true_path_reconstructed_centerlines, segmentation_mask_np)

    true_path_existing_centerlines = cut_mask_from_negative_edges_for_all(true_path_existing_centerlines, segmentation_mask_np)
    true_path_existing_centerlines = clean_paths_on_surface_of_mask(true_path_existing_centerlines, segmentation_mask_np, threshold=0.5)
    ...
    # Negative sampling
    centerlines_train_negative_centerlines = clean_paths_on_surface_of_mask(centerlines_train_negative_centerlines, segmentation_mask_np, threshold=0.5)
    false_edges_classes = [0] * len(centerlines_train_negative_centerlines)

    # Combine positive and negative samples
    train_path_centerlines = centerlines_train_negative_centerlines + true_path_centerlines
    ...
    centerlines_data['train'] = new_train_data
